...
This project is **not compatible with Vercel serverless deployment**.

Why:

- `face_recognition` depends on native `dlib` binaries that are not a good fit for Vercel serverless functions.
- The app stores SQLite data, exported CSV files, and captured face images on disk.
- Vercel functions do not provide the kind of persistent writable filesystem this app needs.

Use one of these instead:

- Render with Docker and a persistent disk
- Railway with Docker and persistent volume support
- VPS or cloud VM
- Your local machine
...
### Recommended cloud deployment: Render
...
5. Make sure the attached disk is mounted at `/var/data/face-attendance`.

Environment and persistence:

- `APP_DATA_DIR=/var/data/face-attendance`
- `APP_LOG_DIR=/var/data/face-attendance/logs`
